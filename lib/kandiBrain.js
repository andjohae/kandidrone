...
kandiBrain.prototype._planRoute = function (dx, dy, cb) {
	// @TODO: Lägg till yaw.

	console.log('planning route...')
	if (!cb) { console.log('Callback to _planRoute is undefined') }
...
kandiBrain.prototype.verifyArguments = function (dx, dy, n, cb) {

	console.log('verifying arguments...');

	if (!cb) { console.log('Callback to verifyArguments is undefined') }
...
kandiBrain.prototype._takeOff = function(h) {
	...
	var self = this;
	this._height = height;
	this._tagSearch.reset(self._tagSearch.resume);
