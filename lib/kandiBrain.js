...
function kandiBrain(client, controller, tagSearch, options) {
	...
	this._lostTag = false;
	this._searchBox = [];
	this._tagLogPath = this._options.path || '../log/tagPositions.txt';
}
...
kandiBrain.prototype.executeRoute = function() {
	...
	this._tagSearch.on('tagConfirmed', function (tagArray) {
		//@TODO: Fix log file for confirmed tag positions
		self._logTag(tagArray[tagArray.length-1]);
	...
	fs.writeFile(self._tagLogPath,
		'The following tag positions were confirmed:\n');
...
kandiBrain.prototype._logTag = function(path, tagPos) {
	fs.appendFile(path, '{x:' + tagPos.x + ', y:' + tagPos.y + '}\n')
}

kandiBrain.prototype.setTagLogPath = function(path) {
	this._tagLogPath = path;
}
