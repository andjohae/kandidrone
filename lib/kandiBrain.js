...
kandiBrain.prototype._planRoute = function (dx, dy, startPos, height, cb, cbObj) {
	...
	var n = Math.floor(2*(dx/width+1)-1); //Totala antalet positioner
	var p = Math.floor(dx/width);
	var restX = dx - p*width;
	var Positions = [];
	...
	if (Y === 0) {
		Positions.push({x: k * width + startX + restX, y: Y + startY});
		Positions.push({x: k * width + startX + restX, y: dy + startY});
	} else {
		Positions.push({x: k * width + startX + restX, y: Y + startY});
		Positions.push({x: k * width + startX + restX, y: startY});
	}
	...
	this._controller.on('goalReached', function () {
		if (self._lostTag) {
			if (count < (self._searchBox.length)) {
				self._searchLostTag(count);
				count++;
			} else if (count === self._goals.length){
				self._followingTag = false;
				self._lostTag = false;
			}
		}
