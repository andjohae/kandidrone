...
EPS_base = 0.50; // Maximum distance from the takeoff position and the base tag
NUM_upd = 5 // Intervall for sending event with updated tag position
...
function tagSearch(client, camera, controller, options) {
    ...
    this._busy = false;
    this._tagArray = []; // [x, y, yaw, n, confirmed]
    this._baseTag = false; // True if we find a base tag
}
...
tagSearch.prototype._updateTagArray = function (tag) {
    ...
    // If tag was not already found, create a new array and send event for new tag
    tagArray.push({x: x, y: y, yaw: yaw, numDetect: 1, confirmed: false});
    self.emit('newTag', tagArray[tagArray.length-1]);
    // Check if a base tag is found
