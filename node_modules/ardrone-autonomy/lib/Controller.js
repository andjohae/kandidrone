...
EPS_LIN      = 0.2; // We are ok with 10 cm horizontal precision
EPS_ALT      = 0.2; // We are ok with 10 cm altitude precision
...
function Controller(client, options) {
    ...
    //  this._pid_x   = new PID(0.1, 0.01, 0.2);
    //  this._pid_y   = new PID(0.1, 0.01, 0.2);
    this._pid_x   = new PID(0.5, 0.1, 0.35);
    this._pid_y   = new PID(0.5, 0.1, 0.35);
...
Controller.prototype._go = function(goal, callback) {
    ...
    /*
        Scale x and y coordinates of the goal to reduce overshoot.
        This was added due to the fact that the state estimation
        tends to drift.
            - kandiDrone team
    */
    goal.x = GOAL_SCALE_X * goal.x;
    goal.y = GOAL_SCALE_Y * goal.y;
...
/* Alternative yaw regulation:
    //Yaw regulation follows f(x) = A*x^($pow) in interval 0 <= |x| <= PI/$denom
    var denom = 8;
    var pow = 1/1.5;
    var A = Math.pow((denom/Math.PI),pow);  //From boundary condition f(PI/denom) = 1

    if (Math.abs(eyaw) < (Math.PI/denom)) {
      if(eyaw > 0) {
        var uyaw = A * Math.pow(eyaw,pow);
      } else if(eyaw < 0) {
        var uyaw = -A * Math.pow(-eyaw,pow);
      } else {
        var uyaw = 0;
...
*/
    ...
    var cx   = scale*within(Math.cos(yaw) * ux + Math.sin(yaw) * uy, -1, 1);
    var cy   = scale*within(-Math.sin(yaw) * ux + Math.cos(yaw) * uy, -1, 1);
    var cz   = scale*within(uz, -1, 1);
